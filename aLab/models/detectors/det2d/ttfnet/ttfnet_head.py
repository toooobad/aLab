...
class TTFNetHead(AnchorFreeHead2D):
    ...
    def get_losses(self, batch_preds: dict, batch_gt_instances: List[InstanceData], batch_metainfos: List[dict]) -> dict:
        ...
        if (self.base_location is None) or (output_h != self.base_location.shape[1]) or (output_w != self.base_location.shape[0]):
            ...
            shifts_x = torch.arange(0, 
                                    (output_w - 1) * base_step_w + 1, 
                                    base_step_w,
                                    dtype=torch.float32, device=heatmap_preds.device)
            shifts_y = torch.arange(0, 
                                    (output_h - 1) * base_step_h + 1, 
                                    base_step_h,
                                    dtype=torch.float32, device=heatmap_preds.device)
            shift_y, shift_x = torch.meshgrid(shifts_y, shifts_x)
            self.base_location = torch.stack((shift_x, shift_y), dim=0)  # (2, h, w)
        
        bbox_preds = torch.cat((self.base_location - batch_preds['bboxes'][:, [0, 1]],
                                self.base_location + batch_preds['bboxes'][:, [2, 3]]), dim=1)  # [bs, 4, h, w]
        ...
        bbox_target_weights = batch_targets['bbox_weights'].permute(0, 2, 3, 1).reshape(-1, 4)  # [bs*h*w, 4]
    ...
    def _get_single_targets(self, gt_instances: InstanceData, metainfo: dict, output_size: Tuple[int]) -> Tuple[Tensor]:
        ...
        bbox_target = gt_bboxes.new_ones((4, output_h, output_w)) * -1
        bbox_target_weight = gt_bboxes.new_zeros((4, output_h, output_w))
        ...
        output_bboxes[:, [0, 2]] = torch.clamp(output_bboxes[:, [0, 2]] / down_sample_w, min=0, max=output_w - 1)
        output_bboxes[:, [1, 3]] = torch.clamp(output_bboxes[:, [1, 3]] / down_sample_h, min=0, max=output_h - 1)
        ...
        output_ctxs = (gt_bboxes[:, 0] + gt_bboxes[:, 2]) / 2 / down_sample_w
        output_ctys = (gt_bboxes[:, 1] + gt_bboxes[:, 3]) / 2 / down_sample_w
        ...
        for obj_id in range(num_objs):
            ...
            bbox_target_weight[:, bbox_target_inds] = (local_heatmap / ct_div)[None, :]
...
class TTFNetPostprocess(CenterNetPostprocess):
    def _process_single_sample(self, single_preds: dict, data_sample: DetDataSample, rescale: bool = False, to_numpy: bool = True) -> DetDataSample:
        ...
        bbox_pred = single_preds['bboxes']
        bbox_pred = bbox_pred.permute(1, 2, 0).reshape(-1, 4)  # [-1, 2]
        ...
        pred_bbox[:, 0] = (grid_coords[:, 0] - bbox_pred[:, 0])
        pred_bbox[:, 1] = (grid_coords[:, 1] - bbox_pred[:, 1])
        pred_bbox[:, 2] = (grid_coords[:, 0] + bbox_pred[:, 2])
        pred_bbox[:, 3] = (grid_coords[:, 1] + bbox_pred[:, 3])
